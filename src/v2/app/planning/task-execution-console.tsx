...
type StageId = 'input' | 'rules' | 'candidates' | 'selection' | 'output'
...
const STAGES: Array<{ id: StageId; title: string; short: string }> = [
  { id: 'input', title: '订单确认', short: '01' },
  { id: 'rules', title: '规则装载', short: '02' },
  { id: 'candidates', title: '候选生成', short: '03' },
  { id: 'selection', title: '方案筛选', short: '04' },
  { id: 'output', title: '结果输出', short: '05' },
]
  ...
  return (
    <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_60px_-40px_rgba(15,23,42,0.35)]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-2xl font-semibold text-slate-950">任务进度</h2>
        </div>
        <div className="flex items-center gap-3">
          <span className={statusBadgeClass(phase)}>{PHASE_LABELS[phase]}</span>
          <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">
            {activeRun ? activeRun.id.slice(0, 8) : '未创建任务'}
          </span>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <div className="space-y-4">
          <div className="rounded-[24px] bg-slate-950 p-5 text-white">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="text-xs uppercase tracking-[0.18em] text-slate-400">当前进度</div>
        ...
        <div className="space-y-4">
          ...
          <SidePanel
            title="已启用规则"
            rows={[
              ['规则项', enabledRules.length > 0 ? enabledRules.join(' / ') : '-'],
              ['偏好评分', runtimeFlags.preference ? '开启' : '关闭'],
              ['测试模式', runtimeFlags.testMode ? '开启' : '关闭'],
              ['长坏料', runtimeFlags.short ? '开启' : '关闭'],
              ['跨钢种', runtimeFlags.crossGroup ? '开启' : '关闭'],
            ]}
          />
        </div>
      </div>
...
function deriveUiState(status: PlanningRunStatus, lastTransitionAt: string | null) {
  ...
  switch (status) {
    case 'draft':
      return { phase: 'draft' as TaskPhase, activeStage: 'input' as StageId, progress: 5, lastRunAt }
    case 'waiting':
      return { phase: 'waiting' as TaskPhase, activeStage: 'input' as StageId, progress: 15, lastRunAt }
    case 'executing':
      return { phase: 'executing' as TaskPhase, activeStage: 'rules' as StageId, progress: 38, lastRunAt }
    case 'processing':
      return {
        phase: 'processing' as TaskPhase,
        activeStage: 'candidates' as StageId,
        progress: 72,
        lastRunAt,
      }
    case 'standby':
    ...
    case 'cancelled':
      return { phase: 'cancelled' as TaskPhase, activeStage: null, progress: 0, lastRunAt }
  }
}
