...
export function PlanningResultsPanel({
  ...
  const filteredDesignResult = useMemo(
    () =>
      designResult.filter((row) => {
        ...
        if (pendingOnly && row.status !== 'pending' && row.status !== 'imported') return false
        return true
      }),
  ...
  const focusedSlabRows = useMemo(
    () => (activeDesign ? slabTable.filter((row) => row.slabNo === activeDesign.slabNo) : slabTable),
  ...
  const slabToPlateGap = (Number(totalSlabWeight) - Number(totalPlateWeight)).toFixed(2)
  const remainWeight = remainOrders
    .reduce((sum, row) => sum + Number(row.remainWeight || 0), 0)
    .toFixed(2)
  const avgPlanScore =
    designResult.length > 0
      ? (designResult.reduce((sum, row) => sum + row.planScore, 0) / designResult.length).toFixed(1)
      : '0.0'

  const isResultReady = run ? run.status === 'standby' || run.status === 'completed' : false
  const isRunning = run ? run.status === 'executing' || run.status === 'processing' : false
  ...
  return (
    <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_60px_-40px_rgba(15,23,42,0.35)]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-2xl font-semibold text-slate-950">结果概览</h2>
        </div>
        <div className="flex flex-wrap gap-2">
          <MetricChip label="方案条目" value={filteredDesignResult.length} tone="blue" />
          <MetricChip label="订单明细" value={plateTable.length} tone="slate" />
          <MetricChip label="板坯汇总" value={slabTable.length} tone="emerald" />
          <MetricChip label="未排订单" value={remainOrders.length} tone="amber" />
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            <span
              className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${
                isResultReady ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
              }`}
            >
              {isResultReady ? '结果已生成' : '结果预览'}
            </span>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-5">
        ...
        <KpiCard label="重量差" value={`${slabToPlateGap} t`} tone="amber" />
        <KpiCard label="平均评分" value={avgPlanScore} tone="blue" />
      </div>

      <div className="mt-6 grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <div className={`rounded-[24px] border border-slate-200 bg-gradient-to-br p-5 text-white ${heroToneClass}`}>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-xs uppercase tracking-[0.18em] text-blue-200/80">主结果</div>
              <h3 className="mt-2 text-xl font-semibold">方案明细</h3>
            </div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            ...
            <HeroMetric label="当前钢种" value={activeDesign?.steel || slabTable[0]?.steel || '-'} />
            <HeroMetric label="未排重量" value={`${remainWeight} t`} />
        ...
        <div className="space-y-4">
          ...
          <SummaryPanel
            title="规则开关"
            rows={[
              ['偏好评分', run?.preference_enabled ? '开启' : '关闭'],
              ['测试模式', run?.test_mode_enabled ? '开启' : '关闭'],
              ['长坏料', run?.short_enabled ? '开启' : '关闭'],
              ['跨钢种', run?.cross_group_enabled ? '开启' : '关闭'],
            ]}
          />
          <SummaryPanel
            title="结果联动"
            rows={[
              ['订单明细', `${focusedPlateRows.length} 行`],
              ['板坯汇总', `${focusedSlabRows.length} 行`],
              ['未排订单', `${focusedRemainRows.length} 行`],
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-3">
        <ResultCard title="订单明细" count={focusedPlateRows.length} accent="slate" active={activeAuxTab === 'plate_table'}>
          <CompactTable
            columns={['订单号', '序列号', '钢种', '块序', '重量', '交付日期']}
        ...
        <ResultCard title="板坯汇总" count={focusedSlabRows.length} accent="emerald" active={activeAuxTab === 'slab_table'}>
          <CompactTable
            columns={['板坯号', '钢种', '板坯规格', '订单数', '模式']}
        ...
        <ResultCard title="未排订单" count={focusedRemainRows.length} accent="amber" active={activeAuxTab === 'remain_orders'}>
          <CompactTable
            columns={['订单号', '序列号', '钢种', '未排重量', '原因']}
          ...
          {rows.slice(0, 8).map((row, index) => {
            ...
            return (
              <tr
                ...
                className={`cursor-pointer transition ${active ? 'bg-white/10' : 'hover:bg-white/6'}`}
              >
                <HeroTd>{String(index + 1).padStart(2, '0')}</HeroTd>
                <HeroTd>{row.ordNo}</HeroTd>
                <HeroTd>{row.ordItem}</HeroTd>
                <HeroTd>
                  <span className={`rounded-full px-2.5 py-1 text-xs ${getStatusToneClass(row.status)}`}>
                ...
                <HeroTd>
                  <span className={`rounded-full px-2.5 py-1 text-xs ${getPriorityToneClass(row.priority)}`}>
...
function buildDesignResultRows(orders: OrderWithSteelGrade[]): DesignResultRow[] {
  return orders.slice(0, 8).map((order, index) => {
    ...
    const priority: DesignResultRow['priority'] = index <= 1 ? 'high' : index <= 4 ? 'medium' : 'low'
...
function buildSlabTableRows(
  ...
  const grouped = new Map<string, { count: number; thickness: number; width: number; length: number }>()
...
function buildRemainOrderRows(orders: OrderWithSteelGrade[]): RemainOrderRow[] {
  return orders
    ...
    .map((order, index) => ({
      ...
      reason: index % 2 === 0 ? '待二次组合' : '等待跨钢种分配',
    }))
}
