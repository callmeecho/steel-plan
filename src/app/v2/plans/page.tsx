...
import { getTaskSnapshot } from '@v2/lib/task-snapshot-store'
...
export default async function V2PlansPage({
  ...
  const { taskId = 'current', tab = 'graph' } = await searchParams
    ...
    <>
      ...
      <div className="flex items-center justify-between border-b border-edge bg-white px-[18px] pt-3 pb-2.5">
        ...
        <div className="flex gap-1.5">
          <Link
            href={`/v2/plans?taskId=${taskId}&tab=${tab}`}
      ...
      <div className="flex gap-1 border-b border-edge bg-white px-[18px] text-[12px]">
        {Object.entries(TAB_LABELS).map(([key, label]) => (
          <Link
            key={key}
            href={`/v2/plans?taskId=${taskId}&tab=${key}`}
            ...
            <table className="w-full text-[12px] tabular-nums">
              ...
              <tbody>
                {(snapshot?.results ?? []).slice(0, 200).map((row) => (
                  <tr key={row.slabNo} className="h-9 border-t border-edge hover:bg-surface-hover">
